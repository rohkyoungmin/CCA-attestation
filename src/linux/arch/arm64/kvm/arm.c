...
int kvm_arch_vcpu_ioctl_run(struct kvm_vcpu *vcpu)
{
	...
	if (run->exit_reason == KVM_EXIT_MMIO) {
		...
		if (ret)
			return ret;
		/*
		 * For Realm VMs: after userspace fills run->mmio.data for a
		 * read, propagate the result to entry.gprs[0] so the RMM can
		 * inject it into the Realm's destination register on re-entry.
		 */
		if (vcpu_is_rec(vcpu) && !run->mmio.is_write &&
		    kvm_vcpu_dabt_isvalid(vcpu)) {
			struct rec *rec = &vcpu->arch.rec;

			rec->run->entry.gprs[0] =
				vcpu_get_reg(vcpu, kvm_vcpu_dabt_get_rd(vcpu));
		}
	}
