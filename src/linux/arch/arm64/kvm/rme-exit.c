...
static int rec_exit_sync_dabt(struct kvm_vcpu *vcpu)
{
	...
	bool isv = kvm_vcpu_dabt_isvalid(vcpu);
	int rd = isv ? kvm_vcpu_dabt_get_rd(vcpu) : 0;
	int ret;

	if (is_write && isv)
		vcpu_set_reg(vcpu, rd, rec->run->exit.gprs[0]);

	ret = kvm_handle_guest_abort(vcpu);

	/*
	 * For in-kernel MMIO reads (e.g. VGIC): propagate the read result
	 * to entry.gprs[0] so the RMM injects it into the Realm's
	 * destination register on re-entry.  Mirrors the sys_reg read path.
	 */
	if (ret > 0 && !is_write && isv)
		rec->run->entry.gprs[0] = vcpu_get_reg(vcpu, rd);
