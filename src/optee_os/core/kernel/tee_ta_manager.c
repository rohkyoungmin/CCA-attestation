...
static void destroy_context(struct tee_ta_ctx *ctx)
{
	...
	int i = 2;
	if(test_addr == 0x630de10)
	{
		//DO breakpoint test in 6296EBC
		//DO watchpoint test in 0x630de10
		//For functional test
		#define SC_AUTH    0x80000FF0
		#define SC_WATCHPOINT    0x80000FF1
		#define SC_BREAKPOINT    0x80000FF2
		#define SC_ETE_TRACE    0x80000FF3
		#define SC_DISABLE_TRACE    0x80000FF4
		#define SC_STEPPING    0x80000FF5
		#define SC_INTROSPECTION 0x80000FF6
		#define SC_EL3_INTROSPECTION 0x80000FF8
		#define SC_DISABLE_STEPPING 0x80000FF9

		ctx->panic_code = 88776655;
		...
		// thread_smc(SC_EL3_INTROSPECTION, w_vaddr, 0x1, 0x0);
		// thread_smc(SC_EL3_INTROSPECTION, w_vaddr, 0x0, 0x1);
		...
		//read test
		uint32_t f = ctx->flags;
		//performance end point
		DMSG("flag val %u", f);
		DMSG("panic_code val %u", ctx->panic_code);
