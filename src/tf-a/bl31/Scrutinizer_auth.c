...
void sha256_print_hash(uint32_t *H, const char *title) {
	NOTICE("-----------------------------------------------------------------------------------------\n");
	NOTICE("|   H[0]   |   H[1]   |   H[2]   |   H[3]   |   H[4]   |   H[5]   |   H[6]   |   H[7]   |\n");
	NOTICE("| 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x |\n", H[0], H[1], H[2], H[3], H[4], H[5], H[6], H[7]);
	NOTICE("-----------------------------------------------------------------------------------------\n");
}
...
int check_auth(uint64_t signature) {
	...
	SHA256_CTX ctx;
	sha256_init(&ctx);
	sha256_update(&ctx, (BYTE* )sig, 32);
	sha256_f(&ctx, user_sig);
	// int i = 0;
	// for(i = 0; i<SHA256_BLOCK_SIZE; i++){
	// 	NOTICE("0x%x, ", user_sig[i]);
	// }
	
	if (memcmp(user_sig, true_sig, SHA256_BLOCK_SIZE) !=0) {
		return 0;
	}
    return 1;
}
