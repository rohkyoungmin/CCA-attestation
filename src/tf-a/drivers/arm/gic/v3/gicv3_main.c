...
//Since PMIs are set as SPIs, ensure the PMI to be routered in the target core by using GIC affinity.
//For FVP. Juno uses GICv2 and GICv2 does not support affinity...
void interrupt_affinity (unsigned int id)
{
    uint64_t gic_affinity_val =
			gicd_irouter_val_from_mpidr(read_mpidr(), 0U);
		gicd_write_irouter(gicv3_driver_data->gicd_base, id,
					gic_affinity_val);
}
