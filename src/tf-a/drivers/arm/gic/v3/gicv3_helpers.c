		...
		unsigned int interrupt_props_num)
{
	...
	for (i = 0U; i < interrupt_props_num; i++) {
		...
		//Related to shelter interrupt isolation extension. Since only the current primiary core inovkes the affinity router, so the SPIs are only routered to primary CPU core.
		//It means that read_mpidr returns the current core's a.b.c.d. Each core has mpidr_el1 and specific affinity val levels a.b.c.d.
		gic_affinity_val =
