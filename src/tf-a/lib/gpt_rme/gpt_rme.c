		       ...
		       size_t l0_mem_size)
{
	...
	// VERBOSE("l0 gpt sum is %lu\n",GPT_L0_REGION_COUNT(gpt_config.t));
...
static gpt_config_t main_gpt_config;
static gpt_config_t ag_gpt_config;

void gpt_ag_build(void)
{
	main_gpt_config = gpt_config;
	pas_region_t pas_regions[] = {
		ARM_PAS_TRUSTED_DRAM,
		ARM_PAS_OTHER_ANY,
		ARM_PAS_KERNEL,
		ARM_PAS_TRBE,
		ARM_PAS_AGENT,
	...
	if (gpt_init_l0_tables(GPCCR_PPS_64GB, ARM_L0_GPT_AG_ADDR_BASE,
		ARM_L0_GPT_AG_SIZE) < 0) {
		ERROR("gpt_ag_init_l0_tables() failed!\n");
		panic();
	}

	/* Carve out defined PAS ranges. */
	if (gpt_init_pas_l1_tables(GPCCR_PGS_4K,
				   ARM_L1_GPT_AG_ADDR_BASE,
				   ARM_L1_GPT_AG_SIZE,
				   pas_regions,
				   (unsigned int)(sizeof(pas_regions) /
				   sizeof(pas_region_t))) < 0) {
		ERROR("gpt_ag_init_pas_l1_tables() failed!\n");
		panic();
	}
	ag_gpt_config = gpt_config;
	gpt_config = main_gpt_config;

	//test
	NOTICE("main gpt l0 base: 0x%lx; ag gpt l0 base: 0x%lx\n", main_gpt_config.plat_gpt_l0_base, ag_gpt_config.plat_gpt_l0_base);
}

//0 main gpt; 1 agent gpt	
void gpt_switch(size_t gpt_id)
{
	if(gpt_id == 0)
		gpt_config = main_gpt_config;
	else
		gpt_config = ag_gpt_config;
	
	if (gpt_enable() < 0) {
		ERROR("switch gpt_enable() failed!\n");
		panic();
	}
}

//one page ransition
static int gpt_transition_pas_page(uint64_t pa, uint64_t size,
			unsigned int target_pas)
{
	gpi_info_t gpi_info;
	uint64_t nse;
...
//pa must allign, size must allign page
void gpt_trans_multi_pages(size_t gpt_id, uint64_t pa, uint64_t size,
			unsigned int target_pas)
{
	gpt_config_t temp_gpt_config = gpt_config;
	if(gpt_id == 0)
		gpt_config = main_gpt_config;
	else
		gpt_config = ag_gpt_config;
	int ret = 0;
    uint64_t base_pa = pa; 
    uint64_t page_number = size / 0x1000;
    int i;
    for(i =0; i< page_number;i++)
    {
        ret = gpt_transition_pas_page(base_pa, 0x1000, target_pas);
		if(ret!=0){
			VERBOSE("TRANS ERROR 0x%lx 0x%lx\n",pa, size);
		}
        base_pa = base_pa + 0x1000;
    }
	gpt_config = temp_gpt_config;
}